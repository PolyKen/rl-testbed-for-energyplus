...
$ git clone https://github.com/PolyKen/rl-testbed-for-energyplus.git
```
...
```
# Run command (example)
# $ time python3 -m run --num-timesteps 1000000000
...
### Running
Simulation process starts by the following command. <br>
The applicable options are <br>
- `--num-timesteps` or `-n`
- `--learn` or `-l`
- `--policy` or `-p`
...
```
$ python3 -m run -n 1000000000 -l default_learn -p DefaultMlpPolicy
```
